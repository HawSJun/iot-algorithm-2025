# da03_main_cost_spanningtree.py
# 최소 비용신장 트리
...
# 특정 정점에 그래프가 연결 확인 함수
def findVertex(g, finVtx):
    stack = []
    visitedAry = []
...
while len(newAry) > (SIZE -1): # 간선의 수가 (정점의 수 - 1) 될때까지 반복
    start = newAry[index][1] # 서울부터 시작
    end = newAry[index][2]  # 광주부터 시작
    saveWeight = newAry[index][0]  # 현재 가중치 저장(복원을 위해)
    ...
    if startYN and endYN: # 둘 다 다른 간선 있으니깐 지금 간선을 지워도 됨
        del(newAry[index])
